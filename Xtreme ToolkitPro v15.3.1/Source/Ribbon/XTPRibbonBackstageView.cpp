...
#define CLEARTYPE_QUALITY	5
	...
	UINT uPointSize = MulDiv(-lf.lfHeight, 72, GetDeviceCaps(::GetDC(NULL), LOGPIXELSY));

	lf.lfHeight = -MulDiv(++uPointSize, GetDeviceCaps(::GetDC(NULL), LOGPIXELSY), 72);
...
void CXTPRibbonBackstageView::DrawControl(CDC* pDC, CXTPControl* pControl)
{
	...
	if (IsCommand(pControl))
	{
		...
		if (pIcon)
		{
			...
			rcText.left += XTPDpiHelper()->ScaleX(pIcon->GetWidth(), 120) + IMAGE_MARGIN;
		}
