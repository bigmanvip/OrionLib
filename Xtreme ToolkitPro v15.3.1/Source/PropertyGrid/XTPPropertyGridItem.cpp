...
void CXTPPropertyGridItem::OnMarkupChanged()
{
	...
	if (m_pGrid->m_pGrid->GetMarkupContext() && GetMarkupTemplate().GetLength())
	{
		...
		//RefreshHeight(); // not enough sometimes
...
void CXTPPropertyGridItem::OnValueChanged(const CString &strValue)
{
	if (!m_pGrid)
		return;
...
{
	if (GetMarkupValue() && m_pGrid && m_pGrid->m_pGrid->GetMarkupContext())
	{
		...
		rcValue.left += 4; // as in CXTPPropertyGridPaintManager::DrawItemValue
		...
		CSize sz = XTPMarkupMeasureElement(GetMarkupValue(), abs(rcValue.Width()), INT_MAX);
